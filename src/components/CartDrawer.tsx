      ...
      <div className="fixed top-0 right-0 bottom-0 z-50 w-full max-w-[420px] bg-[#161B22] border-l border-[#30363D] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#30363D]">
          <h2 className="text-white font-semibold text-lg">
            {lang === 'ar' ? 'سلة التسوق' : lang === 'fr' ? 'Panier' : 'Shopping Cart'} ({items.length})
          </h2>
          <button
            onClick={closeCart}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-[#8B949E] text-sm mt-1">{lang === 'ar' ? 'تصفح منتجاتنا واعثر على دراجتك المثالية.' : lang === 'fr' ? 'Parcourez nos produits et trouvez votre véhicule idéal.' : 'Browse our products and find your perfect ride.'}</p>
              <Link
                to="/store"
                onClick={closeCart}
